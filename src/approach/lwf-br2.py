...
class Appr(Inc_Learning_Appr):
    ...
    def criterion(self, t, outputs, targets, outputs_old=None):
        ...
        # Knowledge distillation loss for all previous tasks
            ...
            for i in range(t):
                g = torch.sigmoid(outputs[i])
                q = torch.sigmoid(outputs_old[i])
                loss += self.lamb * torch.nn.functional.binary_cross_entropy(g, q)

        # Current cross-entropy loss -- with exemplars use all heads
        if len(self.exemplars_dataset) > 0:
            return loss + torch.nn.functional.cross_entropy(torch.cat(outputs, dim=1), targets)
        gt = torch.sigmoid(outputs[t])
        one_hot = torch.nn.functional.one_hot(targets-self.model.task_offset[t],num_classes=gt.shape[1])
        binary = torch.nn.functional.binary_cross_entropy(gt,one_hot.to(torch.float32))
        return loss + binary + torch.nn.functional.cross_entropy(outputs[t], targets - self.model.task_offset[t])
