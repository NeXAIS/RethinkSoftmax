    ...
    def criterion(self, t, outputs, targets, ref_outputs=None, features=None, ref_features=None):
        ...
        if ref_outputs is None or ref_features is None:
            ...
            if self.less_forget:
                ...
                batch_size_per_task = features.shape[0]
                for i in range(t):
                    start_idx = i * batch_size_per_task
                    end_idx = start_idx + batch_size_per_task
                    task_features = features[start_idx:end_idx]
                    ref_task_features = ref_features[start_idx:end_idx].detach()
                    # task_targets = torch.ones(task_features.shape[0],device=self.device)

                    # task_dist = nn.CosineEmbeddingLoss()(
                    #         task_features,
                    #         ref_task_features.detach(),
                    #         task_targets
                    #     )
                    # loss_dist += task_dist

                    # separated distillation loss
                    g = torch.sigmoid(task_features)
                    q = torch.sigmoid(ref_task_features)
                    task_binary = torch.nn.functional.binary_cross_entropy(g,q)
                    loss_dist += task_binary * self.lamda
            else:
            ...
            logits=torch.cat([o['wsigma'] for o in outputs], dim=1)
            gt=torch.sigmoid(logits)
            num_classes=logits.size(1)
            one_hot=F.one_hot(targets, num_classes=num_classes).float()
            binary_loss=F.binary_cross_entropy(gt, one_hot)


            # Eq. 9: integrated objective
            loss = loss_dist + loss_ce + loss_mr + binary_loss
